...
local function _exec_[[NAME]]_cli(...)
    ...
    local _proc = proc.spawn('bin/[[CLI_NAME]]', _arg, {stdio = {stdout = 'pipe', stderr = 'pipe'}, wait = true})
...
local function _get_[[NAME]]_cli_result(exitcode, stdout, stderr)
    if exitcode ~= 0 then
...
if _info.[[DAEMON_NAME]] == 'running' then
    if am.app.get_configuration("NODE_PRIVKEY") then
        local _exitcode, _stdout, _stderr = _exec_[[NAME]]_cli("-datadir=data", "masternode", "status")
        local _success, _output = _get_[[NAME]]_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_[[NAME]]_cli('-datadir=data', 'getblockchaininfo')
    local _success, _output = _get_[[NAME]]_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_[[NAME]]_cli('-datadir=data', 'mnsync', 'status')
    local _success, _output = _get_[[NAME]]_cli_result(_exitcode, _stdout, _stderr)
